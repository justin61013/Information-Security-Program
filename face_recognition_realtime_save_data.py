...
def liveness_detect(mtcnn,resnet,predictor,gallery_emb,gallery_map,index):
    ...
    size=cf.image_size
    #按目前已有的數量作為保存視頻的命名規則
    video = cv2.VideoWriter(os.path.join(cf.saved_path,str(len(os.listdir(cf.saved_path)))+'.mp4'), cv2.VideoWriter_fourcc(*'mp4v'), fps, size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saved-path', type=str, default=os.path.join(os.path.dirname(os.path.realpath(__file__)),'saved_vedio'),
                         help='path of face vedio')

    parser.add_argument('--image-size', type=tuple, default=(1280,960),help='saved vedio size')

    parser.add_argument('--fps', type=int, default=8, help='saved vedio fps')

    parser.add_argument('--liveness-threshold', type=int, default=2, help='threshold for liveness,larger will be harder, as well time will be longer')
